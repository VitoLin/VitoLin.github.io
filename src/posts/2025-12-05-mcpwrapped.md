...
## Background
I was curious about why LLMs seem to never be able to use the MCP tools I gave it. I read [`LLMTest_NeedleInAHaystack`](https://github.com/gkamradt/LLMTest_NeedleInAHaystack) which tested LLM context windows with the following steps:
> 1. Place a random fact or statement (the 'needle') in the middle of a long context window (the 'haystack')
> 2. Ask the model to retrieve this statement
> 3. Iterate over various document depths (where the needle is placed) and context lengths to measure performance

Models lose ability to retrieve **the needle** as the context windows grow and as **the needle** is further up the input
[![Claude 2.1](https://github.com/gkamradt/LLMTest_NeedleInAHaystack/raw/main/img/Claude_2_1_testing.png)](https://github.com/gkamradt/LLMTest_NeedleInAHaystack)

Because of this, MCP tools need to be kept fresh at the end of the context window to keep agent aware of the tools.

With large MCP servers, with dozens of tools each with its own description, the context window gets clogged by information of unused tools which competes with the freshness in the model's memory. These unused tools should not have the same relevance to the model as your prompt or other tools you do want. 

I found this to be extremely annoying so I created this simple wrapper which that goes wherever your mcp servers go and can filter out only the tools you actually want to use.

## How does it work
MCPWrapped is a proxy that sits between the client (your AI tool) and your MCP server of choice. Whenever the client tries to fetch the tools-list, MCPWrapped intercepts it and returns a filtered list.

Here is an example with the [`playwright-mcp-server`](https://github.com/microsoft/playwright-mcp), I am using [mcp-inspector](https://modelcontextprotocol.io/docs/tools/inspector) to get the list of tools and Claude Code's `/context` to see the token count:
```sh
npx @modelcontextprotocol/inspector npx -y @executeautomation/playwright-mcp-server
```

The `playwright-mcp-server` has 33 total tools when realistically just want to give my agent access to viewing the html, navigating, and viewing some logs.
...
This ended up being around 3.6k tokens. For common 128k token models, this would be around ~3% of your context window which is not too bad. For some internal mcp tools at my work, the main monolith mcp server took around ~20k tokens!

Using the [mcpwrapped node package](https://www.npmjs.com/package/mcpwrapped)

```sh
npm install -g mcpwrapped
npx @modelcontextprotocol/inspector mcpwrapped npx -y @executeautomation/playwright-mcp-server --visible_tools=playwright_navigate,playwright_get_visible_html,playwright_console_logs,playwright_click
```
...
This reduced the usage to 848 tokens, ~24% of the size compared using the whole mcp server.


## Usage
Here is an example of `mcpwrapped` in action using the standard [FastMCP](https://gofastmcp.com/integrations/mcp-json-configuration) server configuration:
```json
"mcpServers": {
  "playwright-wrapped": {
    "disabled": false,
...
## But why
The main benefit of trimming down mcp server context is the ability to reuse existing mcp servers and just focus it down to your current tasks. For example, I narrowed down the `playwright-mcp-server` to focus on retrieving the website's html content and console logs for debugging websites without having to rewrite my own. 

Currently this functionality is not widely available. Even popular coding tools like Claude has this open issue: https://github.com/anthropics/claude-code/issues/7328

## Continuation
This is mostly the quick thoughts I had around the topic of MCP tool usage. Some things I want to explore:
> - Pulling down [`LLMTest_NeedleInAHaystack`](https://github.com/gkamradt/LLMTest_NeedleInAHaystack) and modifying it test successful MCP tool usage.
> - Creating a simple tool for tokenizing MCP server context and seeing how the ranking of the most popular MCP servers compares with its context length.
